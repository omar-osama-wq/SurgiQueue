...
class HospitalDashboard:
    ...
    def patient_form(self, title, values=None, update=False):
        ...
        def save():
            try:
                patient_id = entries["id"].get().strip().upper()
                name = entries["name"].get().strip()
                age_text = entries["age"].get().strip()
                diagnosis = entries["diagnosis"].get().strip()
                urgency = entries["urgency"].get().strip()

                if not name or not age_text or not diagnosis or not urgency:
                    raise ValueError("All patient fields are required.")
    ...
    def show_surgeries(self):
        ...
        tree = ttk.Treeview(
            card,
            columns=("id", "patient", "diagnosis", "doctor", "type", "date", "room", "status"),
            show="headings"
        )
        for col, heading, width in [
            ("id", "Patient ID", 110),
    ...
    def add_surgery_window(self):
        ...
        def save():
            ...
            if not all([patient_id, surgery_type, doctor, date]):
                messagebox.showwarning(
                    "Missing Data",
                    "Please fill in all surgery fields."
                )
                return
            ...
            # Keep only rooms that are not occupied by a scheduled surgery.
            occupied_rooms = {
                s.get("room")
                for s in self.get_surgeries()
                if s.get("status") == "Scheduled"
            }
            rooms = [room for room in rooms if room not in occupied_rooms]
    ...
    def complete_by_patient_id(self, patient_id):
        for surgery in self.surgery_manager.surgeries:
            if str(surgery.get("patient_id", "")).strip().upper() == str(patient_id).strip().upper():
    ...
    def delete_by_patient_id(self, patient_id):
        surgeries = self.surgery_manager.surgeries
